...
function addActionsForHtmlUI(){
   // Color Slider Events
   document.getElementById('camera').addEventListener('mousemove', function(ev) { if(ev.buttons == 1){ g_globalAngle = this.value; renderScene();}});
...
function keydown(ev){
   if(ev.keyCode==39 || ev.keyCode == 68){ // 'D' or Right Arrow
      g_camera.right();
   } else if (ev.keyCode==37 || ev.keyCode == 65){ // 'A' or Left Arrow
      g_camera.left();
   } else if (ev.keyCode==38 || ev.keyCode == 87){ // 'W' or Up Arrow
      g_camera.forward();
   } else if (ev.keyCode==40 || ev.keyCode == 83){ // 'S' or Down Arrow
      g_camera.back();
